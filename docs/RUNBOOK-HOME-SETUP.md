...
2. `beeper setup --server --install --email <their beeper email>` → ask the
   user (in chat) for the code Beeper emails them; enter it. Then
   `beeper verify recovery-key` → ask the user to paste their recovery key.
   Heads-up from docs/DEPLOY.md: the CLI currently installs the NIGHTLY
   channel (known risk beeper/cli#21) — remind the user this was accepted,
   or stop and discuss if they hesitate.
3. Mint the Beeper access token on the box (`beeper token create` or the
   CLI's current equivalent; see `beeper --help`).
...
5. Install the systemd user unit (see bootstrap.sh's sed line for the
...
6. Optionally run `claude` on the host once (user signs in) so the box is
